...
const job = new CronJob({
  ...
  onTick: async function () {
    ...
    const item = (
      await this.articles.find().sort({ pubDate: -1 }).limit(1).toArray()
    )[0];
    const maxPublishingDate = item ? item.pubDate : new Date(0);
    const filteredArticles = feed.items.filter(
      (item) => new Date(item.pubDate) > maxPublishingDate
    );
...
const main = async () => {
  ...
  app.get("/articles", async (req, res) => {
    ...
    results.results = await articles
      .find(filterQuery)
